		...
		proxy: {
			"/api": {
				...
				configure: (proxy) => {
					proxy.on(
						"proxyRes",
						(proxyRes: IncomingMessage, req: IncomingMessage) => {
							const devServerUrl = `http://${req.headers.host}`;

							// Transform OAuth redirects to use dev server
							if (proxyRes.headers.location) {
								const location = proxyRes.headers.location;
								const localCallback = encodeURIComponent(
									`${devServerUrl}/api/auth/callback`,
								);
								const originalCallback = encodeURIComponent(
									"https://api.dev.ridelines.xyz/auth/callback",
								);

								const transformedLocation = location
									.replace(
										`redirect_uri=${originalCallback}`,
